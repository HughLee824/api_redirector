...
export async function GET(request: NextRequest) {
  try {
    console.log('[DEBUG] Google Maps geocode JSON route called');
    ...
    // Call Google Maps API with JSON format
    ...
    let proxyResponse;
    if (address) {
      console.log('[DEBUG] Calling geocode with address (JSON format):', address);
      proxyResponse = await googleMapsProxy.geocode(address, 'json', additionalParams);
    } else {
      const [lat, lng] = latlng!.split(',').map(Number);
      console.log('[DEBUG] Calling reverse geocode with coords (JSON format):', lat, lng);
      proxyResponse = await googleMapsProxy.reverseGeocode(lat, lng, 'json', additionalParams);
    }
    
    console.log('[DEBUG] Proxy response status:', proxyResponse.status);

    // Log the request
    Logger.logRequest('GET', `/api/proxy/maps/geocode/json`, apiKey, proxyResponse.status);
    ...
    console.log('[DEBUG] Response data preview:', JSON.stringify(proxyResponse.data).substring(0, 100));
    ...
    return new Response(responseBody, {
      ...
      headers: {
        ...
        'X-Proxy-Service': 'google-maps-json'
      }
    });

  } catch (error) {
    console.log('[DEBUG] Error in geocode JSON route:', error);
    Logger.error('Error in Google Maps geocode JSON proxy', error);
